...
bool Dcu::EnableActuator(const std::string& name) {
  ...
  if (ret == STATE_ENABLE) {
    ...
    LOG_WARN("Actuator %s %d enable failed (state=%d), continuing anyway.", 
             name.c_str(), (int)actr->GetId(), (int)ret);
  }
  return true;
}
...
void Dcu::GetRawCanfdData(CtrlChannel ch, uint8_t* data, size_t* len) {
  std::lock_guard<std::mutex> lock(recv_mtx_);
  size_t channel = static_cast<size_t>(ch);
  if (channel > 2) {
    *len = 0;
    return;
  }
  *len = 64;
  memcpy(data, recv_buf_.canfd[channel], 64);
}
