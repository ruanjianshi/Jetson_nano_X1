...
bool XyberController::SetMode(const std::string& name, ActautorMode mode) {
  auto it = actuator_dcu_map_.find(name);
  if (it == actuator_dcu_map_.end()) return false;

  return it->second->SetMode(name, mode);
}
...
void XyberController::GetRawCanfdData(const std::string& actuator_name, CtrlChannel ch,
                                      uint8_t* data, size_t* len) {
  auto it = actuator_dcu_map_.find(actuator_name);
  if (it == actuator_dcu_map_.end()) {
    *len = 0;
    return;
  }
  it->second->GetRawCanfdData(ch, data, len);
}
