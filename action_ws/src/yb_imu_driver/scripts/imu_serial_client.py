...
"""
========================================
IMU Serial Action Client 示例
========================================
功能说明:
  - 演示如何使用IMU Serial Action Client
  - 支持单次读取、连续读取、校准、配置等操作

使用方法:
  source devel/setup.bash
  python3 imu_serial_client.py
...
import actionlib
import sys
from yb_imu_driver.msg import IMUSerialAction, IMUSerialGoal


class IMUSerialClient:
    """
    IMU Serial Action Client 类
    
    职责:
      1. 连接到IMU Serial Action Server
      2. 发送各种操作请求
      3. 接收和显示结果
    """
    ...
    def __init__(self):
        ...
        构造函数 - 初始化客户端
        """
        rospy.init_node('imu_serial_client')
        
        self.client = actionlib.SimpleActionClient('imu_serial', IMUSerialAction)
        
        rospy.loginfo("正在连接到 IMU Serial Action Server...")
        self.client.wait_for_server()
        rospy.loginfo("✅ 已连接到 IMU Serial Action Server")
    ...
    def interactive_menu(self):
        ...
        while not rospy.is_shutdown():
            print("\n" + "=" * 60)
            print("IMU Serial Action Client - 交互式菜单")
            print("=" * 60)
            print("1. 单次读取")
            print("2. 连续读取 (5秒)")
            print("3. IMU校准 (自动)")
