...
"""
========================================
IMU I2C Action Server
========================================
功能说明:
  - 实现 ROS Action 接口，控制九轴IMU通过I2C通信
  - 支持单次/连续读取传感器数据
...
import time
import threading
from YbImuLib import YbImuI2c
from yb_imu_driver.msg import IMUI2CAction, IMUI2CFeedback, IMUI2CResult
...
class IMUI2CServer:
    """
    IMU I2C Action Server 类
    
    职责:
      1. 初始化 ROS 节点和 YbImuI2c
    ...
    def __init__(self):
        ...
        构造函数 - 初始化服务器
        """
        rospy.init_node('imu_i2c_server')
        rospy.loginfo("正在初始化 IMU I2C Action Server...")
        
        self.i2c_port = rospy.get_param('~i2c_port', 7)
        ...
        rospy.loginfo(f"✅ IMU I2C Action Server 已启动")
        rospy.loginfo(f"   I2C端口: {self.i2c_port}")
        rospy.loginfo(f"   Action名称: imu_i2c")
        rospy.loginfo(f"   实时Topic发布: {'启用' if self.publish_topic else '禁用'}")
        if self.publish_topic:
            rospy.loginfo(f"   Topic: /imu_i2c/data, /imu_i2c/mag, /imu_i2c/temperature")
    ...
    def init_imu(self):
        ...
        """
        try:
            self.imu = YbImuI2c(port=self.i2c_port, debug=self.debug)
    ...
    def init_publishers(self):
        ...
        """
        if self.publish_topic:
            self.imu_pub = rospy.Publisher('/imu_i2c/data', Imu, queue_size=10)
            self.mag_pub = rospy.Publisher('/imu_i2c/mag', Vector3, queue_size=10)
            self.temp_pub = rospy.Publisher('/imu_i2c/temperature', Vector3, queue_size=10)
