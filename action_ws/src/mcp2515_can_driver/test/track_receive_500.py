...
class MCP2515RecvTest:
    def __init__(self, spi_bus=0, spi_device=0, spi_speed=10000000):
    ...
    def init_500k_75pct(self):
        ...
        # 配置位定时 (500kbps, 75%采样点, 8MHz晶振)
        # CNF2=0x43, CNF3=0x01: 500Kbps, 75%采样点, 8TQ
        self.write_reg(REG['CNF1'], 0x00)
        self.write_reg(REG['CNF2'], 0x43)
        self.write_reg(REG['CNF3'], 0x01)
        ...
        # 清所有中断和错误标志
        self.write_reg(REG['CANINTF'], 0x00)
        self.write_reg(REG['EFLG'], 0x00)
        time.sleep(0.1)
        # 清除可能残留的MERRF
        self.write_reg(REG['CANINTF'], 0x80)

        canstat = self.read_reg(REG['CANSTAT'])
        canctrl = self.read_reg(REG['CANCTRL'])
        eflg = self.read_reg(REG['EFLG'])
        print(f"初始化完成: 500kbps")
        print(f"  CANSTAT=0x{canstat:02X}, CANCTRL=0x{canctrl:02X}, EFLG=0x{eflg:02X}")
        return True
    ...
    def receive_frame(self):
        ...
        # 清除对应的接收中断标志
        self.write_reg(REG['CANINTF'], 0x03 & ~(1 << buf))
        return {
            'id': can_id,
            'dlc': dlc,
            'data': data,
    ...
    def run(self):
        if not self.init_500k_75pct():
            return
        print("开始监听 CAN 总线 (500kbps)，按 Ctrl+C 退出...")
        ...
        try:
            while True:
                ...
                if frame:
                    ...
                    last_status_time = time.time()  # 收到数据，重置计时器
                ...
                if now - last_status_time >= status_interval:
                    canstat = self.read_reg(REG['CANSTAT'])
                    eflg = self.read_reg(REG['EFLG'])
                    caninte = self.read_reg(REG['CANINTE'])
                    canintf = self.read_reg(REG['CANINTF'])
                    mode = (canstat >> 5) & 0x07
                    mode_str = {0: 'NORMAL', 1: 'SLEEP', 2: 'LOOPBACK', 3: 'LISTEN', 4: 'CONFIG'}.get(mode, 'UNKNOWN')
                    print(f"[状态] CANSTAT=0x{canstat:02X} mode={mode_str} EFLG=0x{eflg:02X} CANINTF=0x{canintf:02X}")
                    if eflg != 0:
                        if eflg & 0x01: print("  RX0OVR - RX buffer 0 overflow")
                        if eflg & 0x02: print("  RX1OVR - RX buffer 1 overflow")
                        if eflg & 0x04: print("  TXBO - Bus-off")
                        if eflg & 0x08: print("  TXEP - TX error passive")
                        if eflg & 0x10: print("  RXEP - RX error passive")
                        if eflg & 0x20: print("  EWARN - Error warning")
                        if eflg & 0x40: print("  RXWRN - RX warning")
                        if eflg & 0x80: print("  TXWRN - TX warning")
                    if canintf & 0x80:
                        print("  MERRF - Message error")
