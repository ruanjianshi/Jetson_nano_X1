...
check_opencv() {
    ...
    if [ -d "/home/jetson/opencv" ]; then
        echo "✓ /home/jetson/opencv 已存在 (218MB)"
    else
        echo -e "${RED}✗ /home/jetson/opencv 不存在${NC}"
        exit 1
    fi
    echo ""
}
...
# 检查contrib zip文件
check_contrib() {
    echo "[检查] opencv_contrib.zip..."
    
    OPENCV_CONTRIB_ZIP="/home/jetson/Downloads/opencv_contrib-4.13.0.zip"
    
    if [ -f "$OPENCV_CONTRIB_ZIP" ]; then
        SIZE=$(du -h "$OPENCV_CONTRIB_ZIP" | cut -f1)
        echo "✓ 找到 opencv_contrib-4.13.0.zip ($SIZE)"
    else
        echo -e "${RED}✗ opencv_contrib-4.13.0.zip 不存在${NC}"
        echo "  请将文件上传到 /home/jetson/Downloads/ 目录"
        exit 1
    fi
    echo ""
}
...
if [ -d "opencv_contrib" ]; then
    ...
    echo "解压opencv_contrib-4.13.0.zip..."
    unzip -q "/home/jetson/Downloads/opencv_contrib-4.13.0.zip"
    mv opencv_contrib-4.13.0 opencv_contrib
    echo "✓ opencv_contrib解压完成"
fi
echo ""
...
# 步骤4: 创建build目录并配置CMake
echo "[步骤 3/4] 配置CMake..."
echo "  启用CUDA, cuDNN等加速功能"
echo ""

cd /home/jetson/opencv
mkdir -p build
cd build
...
echo "      sudo rm -rf /home/jetson/opencv_contrib"
echo ""
echo "建议: 如果不再需要编译缓存，可以删除 ~/opencv/build 目录"
echo ""